...
```
#### 由遍历序列确定二叉树
给定中序序列，和前序，后序，遍历序列的其中一个，就可以唯一确定二叉树。因为中序序列是左子树+根+右子树，而其余的给出根，因此可以把中序序列的三个成分分开，并且由其他的序列给出左右子树的根，这样一直进行，就可以把二叉树唯一确定。
...
#### 规定：
- 若结点无左子树，则令lchild指向其前驱，并令标识指针ltag为1；否则lchild指向左孩子，标识指针ltag为0。
